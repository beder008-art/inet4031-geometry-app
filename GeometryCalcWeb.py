...
def mainForm():
    if request.method == "POST":
        # read a single selection field from the form (only one radio selected)
        selection = request.form.get("shape")
        print("Selection was:", selection)  # prints to command line for trouble shooting
        if selection == "sphere":
...
def sphereForm():
    if request.method == "POST":
        # getting input with name = fname in HTML form
        radius = request.form.get("rad")
        # compute volume for sphere (single radius)
